...
int main()
{
	while(1){
		...
		int power, turn;
		FILE* fp = fopen("/home/pi/status", "r");
		char* temp = (char*)malloc(sizeof(char) * 1024);
		if(fgets(temp, 1024, fp) > 1){
			printf("%s\n", temp);
			sscanf(temp, "%d %d", &power, &turn);
		}else{
			power = turn = 0;
		}
		fclose(fp);
		sprintf(temp, "power=%d&turn=%d", power, turn);
		printf("%s\n", temp);
		while(1)
		{
			...
			if ( recv_message == NULL ){
				...
				if(strstr(recv_message, "power") != NULL){
					fprintf(stdout, "POWER IN");
					sscanf(strstr(recv_message, "=") + 1, "%d", &power);
				}else if(strstr(recv_message, "turn") != NULL){
					fprintf(stdout, "TURN IN");
					if(strstr(recv_message, "al") != NULL)
						turn = 0;
					else
						turn = 1;
				}
			}
			//write_server(client, temp);
			//write_server(client, recv_message);
			fp = fopen("/home/pi/status", "w");
			fprintf(fp, "%d %d", power, turn);
			fprintf(stdout, "%d %d", power, turn);
			fclose(fp);
		}
